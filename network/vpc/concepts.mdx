...
- The **suffix**, which consists of a slash and a number that represents the network size. This is also known as the prefix length or subnet mask bit length. It states how many bits of the prefix/IP address are used to designate the network itself, with the remaining bits therefore being available to designate specific hosts on the network.
